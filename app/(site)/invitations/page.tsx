...
export default async function InvitationsPage() {
  ...
  const { data: invites } = await supabase
    ...
    .select("id, organization_id, email, role, created_at, expires_at, revoked_at, accepted_at, invited_by, organizations(name)")
    .eq("email", userEmail)
    .is("accepted_at", null)
    .is("revoked_at", null)
          ...
          {invites?.map((inv) => (
            ...
            >
              <div className="flex flex-col">
                ...
                {inv.invited_by && (
                  <span className="text-xs text-muted-foreground">
                    Invited by: {inviterProfiles[inv.invited_by]?.full_name || inviterProfiles[inv.invited_by]?.email || inv.invited_by}
                  </span>
                )}
              </div>
