...
const statusColumns = [
  ...
  { id: "in_progress", title: "In Progress", bgColor: "bg-blue-50 dark:bg-blue-900/20" },
  { id: "review", title: "Review", bgColor: "bg-yellow-50 dark:bg-yellow-900/20" },
  { id: "completed", title: "Completed", bgColor: "bg-green-50 dark:bg-green-900/20" },
];

export function DraggableTasksBoard({ initialTasks, projectId }: DraggableTasksBoardProps) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (overColumn) {
      ...
      if (activeTask.status === targetStatus) {
        ...
        const activeIndex = tasksInTargetStatus.findIndex(t => t.id === active.id);
        if (activeIndex !== overIndex) {
          const reorderedTasks = arrayMove(tasksInTargetStatus, activeIndex, overIndex);
    ...
    const taskMoved = originalTask && (originalTask.status !== targetStatus || originalTask.position !== targetPosition);
      ...
      updatePromise
        .then((result) => {
          ...
          if (!result.success) {
            ...
            toast.success(`Task moved to ${statusColumns.find(c => c.id === targetStatus)?.title}`);
