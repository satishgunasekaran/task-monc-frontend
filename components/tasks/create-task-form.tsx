...
import { createTask, updateTask } from "@/app/(site)/tasks/actions";
...
export function CreateTaskForm({ projectId, task, trigger }: CreateTaskFormProps) {
  const isEdit = !!task;
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error(`Failed to ${isEdit ? 'update' : 'create'} task:`, result.error);
        const msg = result.error || `Failed to ${isEdit ? 'update' : 'create'} task`;
        toast.error(msg);
      }
    } catch (error) {
